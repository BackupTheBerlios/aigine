...
#ifdef _DEMO_
	MessageBox(NULL,
		       "Herzlich Willkommen zur ersten Demo von \"Galactica\"!\n\nSie werden gleich den TriBase-Konfigurationsdialog sehen.\nWählen Sie dort am besten eine Auflösung von 800x600 oder höher aus.\nWenn das Spiel gestartet wird, klicken Sie auf \"Steuerung\",\num sich mit den Raumschiffkontrollen vertraut zu machen!\n\nViel Spaß!\nDavid Scherfgen\nhttp://www.scherfgen-software.de",
			   "Information",
			   MB_OK | MB_ICONINFORMATION);
#endif

	// Spiel initialisieren
	g_pGalactica = new CGalactica;
	r = g_pGalactica->Init();
	if(r == TB_CANCELED)
	{
		// Der Konfigurationsdialog wurde abgebrochen!
		// Das Programm "leise" verlassen.
		TB_SAFE_DELETE(g_pGalactica);
		return 0;
	}
	else if(r)
	{
...
// __________________________________________________________________
// Lädt das Spiel
tbResult CGalactica::Load()
{
	// Direct3D initialisieren
	if(tbDirect3D::Init(&m_Config, "Galactica",
...
tbResult Move(float fTime) {return g_pGalactica->Move(fTime);}
tbResult Render(float fTime) {return g_pGalactica->Render(fTime);}

// __________________________________________________________________
// Lässt das Spiel laufen
tbResult CGalactica::Run()
{
...
tbResult CGalactica::SetGameState(EGameState NewGameState)
{
	tbResult r = TB_OK;
...
// __________________________________________________________________
// Bewegt das Spiel
tbResult CGalactica::Move(float fTime)
{
...
// __________________________________________________________________
// Rendert das Spiel
tbResult CGalactica::Render(float fTime)
{
	tbResult r = TB_OK;
