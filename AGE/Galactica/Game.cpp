...
tbResult CGame::Init()
{
	...
	for(int i = 0; i < 32; i++)
	{
		if(g_aiTeam[0][i] != -1)
		{
			iShip = CreateShip(0, g_aiTeam[0][i]);
	...
	g_pGalactica->m_pBriefing->Stop();
	g_pGalactica->m_pAction->Play(TRUE);
...
tbResult CGame::Move(float fTime)
{
	...
	if(g_pbButtons[TB_KEY_ESCAPE]) g_pGalactica->SetGameState(GS_MAIN_MENU);
	...
	// Musik verarbeiten
	g_pGalactica->m_pAction->Process();
...
tbResult CGame::RenderStarfield(float fTime)
{
	...
	srand((int)(g_pGalactica->m_fTime * 10000.0f) % 10000);
...
tbResult CGame::RenderCockpit(float fTime)
{
	...
	g_pGalactica->m_pFont2->DrawText(tbVector2(0.17f, 0.86f), acText, TB_FF_RELATIVE | TB_FF_RELATIVESCALING, -1, Color, Color);
	...
	g_pGalactica->m_pFont2->DrawText(tbVector2(0.17f, 0.89f), acText, TB_FF_RELATIVE | TB_FF_RELATIVESCALING, -1, Color, Color);
	...
	g_pGalactica->m_pFont2->DrawText(tbVector2(0.17f, 0.92f), acText, TB_FF_RELATIVE | TB_FF_RELATIVESCALING, -1, Color, Color);
	...
	g_pGalactica->m_pFont2->DrawText(tbVector2(0.17f, 0.95f), acText, TB_FF_RELATIVE | TB_FF_RELATIVESCALING, -1, Color, Color);
