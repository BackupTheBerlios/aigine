						 ...
						 tbGUI* pGUI)
{
	...
	case TB_GMT_ELEMENTCLICKED:
		...
		case 101: // Spiel starten
			...
			for(int t = 0; t < 4; t++) for(int s = 0; s < 32; s++) g_aiTeam[t][s] = -1;
...
tbResult CMainMenu::Load()
{
	...
	m_pBackground = tbTextureManager::GetTexture("Data\\MainMenu.jpg", TRUE, D3DX_DEFAULT, D3DX_DEFAULT, 1);
	if(m_pBackground == NULL) TB_ERROR("Fehler beim Laden von Data\\MainMenu.jpg!", TB_ERROR);
	...
	m_pGUI->CreateCheckBox(204, 1, tbVector2(450.0f, 70.0f), tbVector2(32.0f), "Joystick benutzen", g_pGalactica->m_pGame->m_bUseJoystick);
...
tbResult CMainMenu::Move(float fTime)
{
	...
	{
		// Spiel starten
		g_pGalactica->SetGameState(GS_GAME);
	}
