/* Generated by Together */

#include "ageIM_Keyboard.h"
#include "ageInput/ageIM_Keyboard.h"
bool ageIM_Keyboard::keysPressed() {
	if(this->keyCount > 0) {
		return true;
	}
	return false;
}
bool ageIM_Keyboard::isKeyPressed(unsigned char key) {
	if(this->keys[key] == 0) {
		return false;
	} else {
		return true;
	}
}
void ageIM_Keyboard::keyActionUp(unsigned char key, int mouseX, int mouseY) {
	this->keyCount--;
	this->keys[key] = false;
}
void ageIM_Keyboard::keyAction(unsigned char key, int mouseX, int mouseY){
	if(key ==  27) exit(0); // ESC for program quit

/*
	if(key == 'w') this->sceneMan->getCamera()->moveForward(1.0);
	if(key == 's') this->sceneMan->getCamera()->moveBack(1.0);
	if(key == 'a') this->sceneMan->getCamera()->moveLeft(1.0);
	if(key == 'd') this->sceneMan->getCamera()->moveRight(1.0);
	if(key == 'n') this->sceneMan->getCamera()->setViewFogMode(true,254.0, 236.0, 208.0, 1.0, 0.2, 100.0, 160.0, GL_LINEAR,GL_FOG_HINT, GL_DONT_CARE);//spaeter ueber einstellungsdialog bzw inidatei
    if(key == 'N') this->sceneMan->getCamera()->setViewFogMode(false,0.0, 0.0, 0.0, 1.0, 0.2, 30.0, 100.0, GL_LINEAR,GL_FOG_HINT, GL_DONT_CARE);
*/
	
	this->keyCount++;
	this->keys[key] = true;
//	this->function();

//	glutPostRedisplay();
}
ageIM_Keyboard::ageIM_Keyboard(void (*func)())
{
	this->function = func;
	this->keyCount = 0;
}
