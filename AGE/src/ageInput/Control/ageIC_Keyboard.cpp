...
#include "ageIC_Keyboard.h"
#include "ageInput/ageIC_Keyboard.h"
bool ageIC_Keyboard::keysPressed() {
	if(this->keyCount > 0) {
		return true;
	}
	return false;
}
bool ageIC_Keyboard::isKeyPressed(unsigned char key) {
	if(this->keys[key] == 0) {
		return false;
	} else {
		return true;
...
void ageIC_Keyboard::keyActionUp(unsigned char key, int mouseX, int mouseY) {
	this->keyCount--;
	this->keys[key] = false;
}
void ageIC_Keyboard::keyAction(unsigned char key, int mouseX, int mouseY){
...
ageIC_Keyboard::ageIC_Keyboard(void (*func)())
{
	this->function = func;
	this->keyCount = 0;
}
