...
private:
	...
	static ageD_TextureListEntry*	m_pTextureList;	// Die Texturliste

public:
	// Methoden
	static age_Result	Init(int iInitialListSize = 256);					// Initialisierung
	static age_Result Exit();												// Herunterfahren
	static age_Result	SetListSize(int iNewSize);							// Verändert die Listengröße
	...
	static age_Result	ReleaseTexture(PDIRECT3DBASETEXTURE9 pTexture);		// Gibt eine Textur frei
	static age_Result	DeleteAllTextures();								// Löscht alle Texturen
	...
	static inline ageD_TextureListEntry*	GetTextureList()		{return m_pTextureList;}
