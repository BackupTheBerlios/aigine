...
#include "ageG_Light.h"
#include "ageG_Vector3.h"
ageG_GraphicPipeline::ageG_GraphicPipeline()
{
	this->windowSize = new int ;
	//this->InitMask();		//	zuerst die bilder mask.zip entpacken!!!
}
ageG_GraphicPipeline::ageG_GraphicPipeline(int argc, char** argv)
{
	GraphicPipeline();
...
void ageG_GraphicPipeline::reshape(int width, int height)
{
...
void ageG_GraphicPipeline::initOpenGL(int argc, char** argv){
...
void ageG_GraphicPipeline::initDisplay(){	
   glShadeModel (GL_SMOOTH);
   glEnable(GL_DEPTH_TEST);
...
ageG_Camera* ageG_GraphicPipeline::getCamera(){
	return this->lnkCamera;
}
void ageG_GraphicPipeline::addLightAt(ageG_Vector3 * position, ageG_Vector3 * lookAt) {
	
	
}
int * ageG_GraphicPipeline::getWindowSize() {
	return this->windowSize;
}
void ageG_GraphicPipeline::setCameraRotation(int mouseX, int mouseY) {
...
int ageG_GraphicPipeline::enableLight(ageG_Light * currentLight, int index) {
	int isSet = false;					
