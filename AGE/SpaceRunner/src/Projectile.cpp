						   ...
						   &vCollision))
		{
			...
			else
			{
				if(pShip->IsVisible())
				{
					...
					if(tbVector3LengthSq(g_pSpaceRunner->m_pGame->m_vCameraPos - pShip->m_vPosition) <= 1000000.0f)
					{
...
tbResult CProjectile::Render(float fTime)
{
	...
	if(m_pType->bIsLaserWeapon)
	{
		...
		{
			// Normales Sprite
			g_pSpaceRunner->m_pGame->m_pSprites->AddSprite(m_pType->iLaserSprite,
				                                         m_vPosition,
		...
		{
			// Laser-Sprite
			g_pSpaceRunner->m_pGame->m_pSprites->AddLaserSprite(m_pType->iLaserSprite,
				                                              m_vPosition,
	...
	else
	{
		...
		if(tbFloatRandom(0.0f, 1.0f) > 0.1f &&
		   !g_pSpaceRunner->m_pGame->m_bPaused)
		{
			g_pSpaceRunner->m_pGame->m_pPSystem->AddParticle(m_pType->fMissileSmokeTrailLifetime + tbFloatRandom(-0.5f, 0.5f),
	...
	g_pSpaceRunner->m_pGame->m_pSprites->AddSprite(m_pType->iGlowSprite,
				                                 vGlowPosition,
												 m_pType->GlowColor,
