...
class TRIBASE_API tbIndexBuffer
{
private:
	// Variablen
...
public:
	...
	// Methoden
	tbResult Init(DWORD dwSize, DWORD dwIndexSize, D3DFORMAT IndexFormat, DWORD dwUsage = D3DUSAGE_WRITEONLY, D3DPOOL Pool = D3DPOOL_DEFAULT);	// Initialisierung
	tbResult Init(PDIRECT3DINDEXBUFFER9 pIndexBuffer, DWORD dwIndexSize);																		// Initialisierung aus einem Index-Buffer
	tbResult Update();																															// Speichert die Änderungen im Index-Buffer
	tbResult SetSize(DWORD dwNewSize);																											// Setzt die Größe des Index-Buffers
	...
	inline tbResult GetIndex(const DWORD dwIndex,
		                     void* pOut)
	{
#ifdef _DEBUG
		// Parameter prüfen
	...
	inline tbResult GetIndices(const DWORD dwStart,
	...
	// Setzen eines Index
	inline tbResult SetIndex(const DWORD dwIndex,
							 const void* pIndex)
	{
#ifdef _DEBUG
		// Parameter prüfen
	...
	inline tbResult SetIndices(const DWORD dwStart,
							   const DWORD dwNumIndices,
							   const void* pIndices)
	{
#ifdef _DEBUG
