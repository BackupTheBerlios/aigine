...
tbGUI::~tbGUI()
{
	...
	// Speicherplatz für die analogen Knöpfe löschen
					 ...
					 tbVector2 vScaling) // = tbVector2(1.0f)
{
	...
	// Vertex-Buffer für 1200 Vertizes erstellen
	...
	// Platz für alle analogen Knöpfe reservieren
	...
	// Den Skin, die Nachrichtenfunktion, die Größe und die Skalierung kopieren
	m_Skin = *pSkin;
	m_pMsgProc = pMsgProc;
	m_vSize = vSize;
	m_vScaling = vScaling;
					 ...
					 float* pfButtons)
{
	...
	// Status der analogen Knöpfe abfragen, alten Status kopieren
	...
	// Nachsehen, ob eine Taste auf der Tastatur gedrückt oder losgelassen wurde
...
tbResult tbGUI::Render(const float fTime)
{
	...
	// Alle Durchgänge rendern (für Mauszeiger)
						...
						tbGUIVertex* pOut) // = NULL
{
	...
	if(pOut == NULL)
	{
		...
		{
			// Vertex-Buffer vergrößern
			if(m_pVertexBuffer->SetSize(m_pVertexBuffer->GetSize() +
										1200 * sizeof(tbGUIVertex)))
			{
				// Fehler!
				TB_ERROR("Fehler beim Vergrößern des Vertex-Buffers!", TB_ERROR);
			}
		}
