...
// ******************************************************************
// Interne Variablen
DWORD	tb_g_adwCRC32Table[256];		// CRC32-Tabelle für die Entschlüsselung
BOOL	tb_g_bCRC32TableFilled = FALSE;	// Ist die Tabelle gefüllt?
										 ...
										 void** ppOut)
{
	...
	{
		fclose(pZIPFile);
		TB_ERROR("Komprimiertes Objekt gefunden! Dekomprimierung wird nicht unterstützt!", TB_ERROR);
	}
	...
	if(bEncrypted)
	{
		...
		{
			fclose(pZIPFile);
			TB_ERROR("Fehler beim Lesen des Verschlüsselungs-Headers!", TB_ERROR);
		}
	}
	...
	if(bEncrypted)
	{
		                      ...
		                      (BYTE*)(pData), pcPassword))
		{
			tbMemFree(pData);
			fclose(pZIPFile);
			TB_ERROR("Entschlüsselung fehlgeschlagen!", TB_ERROR);
		}
	}
