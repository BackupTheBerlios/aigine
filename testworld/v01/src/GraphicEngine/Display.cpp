...
Display::Display(IOControl *ioTmp)
{
	 _IoTmp = ioTmp;
}
Display::~Display()
...
void Display::display_cube() {
   ...
   _IoTmp->setTemp_angle(_IoTmp->getAngle()) ;
   glRotatef(this->_IoTmp->getTemp_angle()->x * ANGULAR_SCALE, 1.0, 0.0, 0.0);
   glRotatef(this->_IoTmp->getTemp_angle()->y * ANGULAR_SCALE, 0.0, 1.0, 0.0);
   glRotatef(this->_IoTmp->getTemp_angle()->z * ANGULAR_SCALE, 0.0, 0.0, 1.0);
