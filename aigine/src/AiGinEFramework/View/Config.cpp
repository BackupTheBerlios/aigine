...
Result DeleteListData(HWND hDlg,
						int iDialogItem)
{
	int		iNumElements;
	void*	pElementData;
	...
	if(iNumElements == CB_ERR) return AGE_ERROR;
	...
	for(int iElement = 0; iElement < iNumElements; iElement++)
	{
		...
		if(pElementData != (void*)(CB_ERR)) AGE_SAFE_MEMFREE(pElementData);
	}
...
Result EnumAdapters(HWND hDlg)
{
	...
	// Liste löschen
	DeleteListData(hDlg, IDC_D3DC_ADAPTERS);
	...
	for(int iAdapter = 0; iAdapter < iNumAdapters; iAdapter++)
	{
		...
		SendDlgItemMessage(hDlg, IDC_D3DC_ADAPTERS, CB_ADDSTRING,
			               0, (LPARAM)(AdapterInfo.Description));
	}

	// Den ersten Adapter in der Liste auswählen
	SendDlgItemMessage(hDlg, IDC_D3DC_ADAPTERS, CB_SETCURSEL, 0, 0);
...
Result EnumDeviceTypes(HWND hDlg)
{
	...
	iSelectedAdapter = (int)(SendDlgItemMessage(hDlg, IDC_D3DC_ADAPTERS, CB_GETCURSEL, 0, 0));

	// Gerätetypenliste leeren
	DeleteListData(hDlg, IDC_D3DC_DEVICE_TYPES);
	...
	for(int iDeviceType = 0; iDeviceType < 3; iDeviceType++)
	{
		...
		{
			iIndex = (int)(SendDlgItemMessage(hDlg, IDC_D3DC_DEVICE_TYPES, CB_ADDSTRING,
			...
			pDeviceType = (D3DDEVTYPE*)(MemAlloc(sizeof(D3DDEVTYPE)));
			*pDeviceType = aDeviceType[iDeviceType];
			
			// Daten dem Listeneintrag zuweisen
			SendDlgItemMessage(hDlg, IDC_D3DC_DEVICE_TYPES, CB_SETITEMDATA,
				               iIndex, (LPARAM)(pDeviceType));

			if(!(Caps.DevCaps & D3DDEVCAPS_HWTRANSFORMANDLIGHT))
			{
				g_TempConfig.Direct3D.dwFlags = D3DCREATE_SOFTWARE_VERTEXPROCESSING;
				EnableWindow(GetDlgItem(hDlg, IDC_D3DC_HARDWARE_TNL), FALSE);
			}
		}
	}

	// Ersten Gerätetyp auswählen
	SendDlgItemMessage(hDlg, IDC_D3DC_DEVICE_TYPES, CB_SETCURSEL, 0, 0);
...
Result EnumVideoModes(HWND hDlg)
{
	int				iSelectedAdapter;
	int				iNumVideoModes;
	...
	DeleteListData(hDlg, IDC_D3DC_VIDEO_MODES);
	...
	for(int iFormat = 0; iFormat < 3; iFormat++)
	{
		...
		for(int iVideoMode = 0; iVideoMode < iNumVideoModes; iVideoMode++)
		{
			...
			pVideoMode = (D3DDISPLAYMODE*)(MemAlloc(sizeof(D3DDISPLAYMODE)));

			// Informationen abfragen
			g_pD3DTemp->EnumAdapterModes(iSelectedAdapter, aFormat[iFormat], iVideoMode, pVideoMode);

			// Texeschreibung für diesen Videomodus generieren
			GetD3DFormatString(pVideoMode->Format, acFormatString);
			...
			iIndex = (int)(SendDlgItemMessage(hDlg, IDC_D3DC_VIDEO_MODES, CB_ADDSTRING,
								              0, (LPARAM)(acVideoModeString)));

			// Daten dem Eintrag zuweisen
			SendDlgItemMessage(hDlg, IDC_D3DC_VIDEO_MODES, CB_SETITEMDATA,
				               iIndex, (LPARAM)(pVideoMode));
		}
	}

	// Ersten Videomodus auswählen
	SendDlgItemMessage(hDlg, IDC_D3DC_VIDEO_MODES, CB_SETCURSEL, 0, 0);
...
Result EnumBackBufferFormats(HWND hDlg)
{
	int				iSelectedAdapter;
	...
	iSelectedAdapter = (int)(SendDlgItemMessage(hDlg, IDC_D3DC_ADAPTERS, CB_GETCURSEL, 0, 0));
	iSelectedDeviceType = (int)(SendDlgItemMessage(hDlg, IDC_D3DC_DEVICE_TYPES, CB_GETCURSEL, 0, 0));
	pSelectedDeviceType = (D3DDEVTYPE*)(SendDlgItemMessage(hDlg, IDC_D3DC_DEVICE_TYPES, CB_GETITEMDATA, iSelectedDeviceType, 0));
	iSelectedVideoMode = (int)(SendDlgItemMessage(hDlg, IDC_D3DC_VIDEO_MODES, CB_GETCURSEL, 0, 0));
	pSelectedVideoMode = (D3DDISPLAYMODE*)(SendDlgItemMessage(hDlg, IDC_D3DC_VIDEO_MODES, CB_GETITEMDATA, iSelectedVideoMode, 0));
	bWindowed = IsDlgButtonChecked(hDlg, IDC_D3DC_WINDOWED_MODE) == BST_CHECKED;

	// Liste leeren
	DeleteListData(hDlg, IDC_D3DC_BACK_BUFFER_FORMATS);
												   ...
												   aFormat[iFormat])))
		{
													 ...
													 bWindowed)))
			{
				...
				if(bHardwareFormatConversion)
				{
					...
					GetD3DFormatString(aFormat[iFormat], acBackBufferString);
					if(bWindowed)
					{
					...
					iIndex = (int)(SendDlgItemMessage(hDlg, IDC_D3DC_BACK_BUFFER_FORMATS, CB_ADDSTRING,
													  0, (LPARAM)(acBackBufferString)));

					// Dem Eintrag das Format zuweisen
					pFormat = (D3DFORMAT*)(MemAlloc(sizeof(D3DFORMAT)));
					*pFormat = aFormat[iFormat];
					SendDlgItemMessage(hDlg, IDC_D3DC_BACK_BUFFER_FORMATS, CB_SETITEMDATA,
	...
	SendDlgItemMessage(hDlg, IDC_D3DC_BACK_BUFFER_FORMATS, CB_SETCURSEL, 0, 0);
...
Result EnumZStencilBufferFormats(HWND hDlg)
{
	int				iSelectedAdapter;
	int				iSelectedDeviceType;
	D3DDEVTYPE*		pSelectedDeviceType;
	...
	pSelectedVideoMode = (D3DDISPLAYMODE*)(SendDlgItemMessage(hDlg, IDC_D3DC_VIDEO_MODES, CB_GETITEMDATA, iSelectedVideoMode, 0));
	bWindowed = IsDlgButtonChecked(hDlg, IDC_D3DC_WINDOWED_MODE) == BST_CHECKED;
	iSelectedBackBufferFormat = (int)(SendDlgItemMessage(hDlg, IDC_D3DC_BACK_BUFFER_FORMATS, CB_GETCURSEL, 0, 0));
	pSelectedBackBufferFormat = (D3DFORMAT*)(SendDlgItemMessage(hDlg, IDC_D3DC_BACK_BUFFER_FORMATS, CB_GETITEMDATA, iSelectedBackBufferFormat, 0));

	// Liste leeren
	DeleteListData(hDlg, IDC_D3DC_Z_STENCIL_BUFFER_FORMATS);
	...
	for(int iFormat = 0; iFormat < 7; iFormat++)
	{
											   ...
											   aFormat[iFormat])))
		{
			...
			{
				// Formattext abfragen und Text generieren
				GetD3DFormatString(aFormat[iFormat], acZStencilBufferString);

				// Listeneintrag generieren
				iIndex = (int)(SendDlgItemMessage(hDlg, IDC_D3DC_Z_STENCIL_BUFFER_FORMATS, CB_ADDSTRING,
												  0, (LPARAM)(acZStencilBufferString)));

				// Dem Eintrag das Format zuweisen
				pFormat = (D3DFORMAT*)(MemAlloc(sizeof(D3DFORMAT)));
				*pFormat = aFormat[iFormat];
				SendDlgItemMessage(hDlg, IDC_D3DC_Z_STENCIL_BUFFER_FORMATS, CB_SETITEMDATA,
								   iIndex, (LPARAM)(pFormat));
			}
		}
	}
	...
	SendDlgItemMessage(hDlg, IDC_D3DC_Z_STENCIL_BUFFER_FORMATS, CB_SETCURSEL, 0, 0);
...
Result EnumMultiSamplingTypes(HWND hDlg)
{
	int						iSelectedAdapter;
	int						iSelectedDeviceType;
	D3DDEVTYPE*				pSelectedDeviceType;
	...
	iSelectedBackBufferFormat = (int)(SendDlgItemMessage(hDlg, IDC_D3DC_BACK_BUFFER_FORMATS, CB_GETCURSEL, 0, 0));
	pSelectedBackBufferFormat = (D3DFORMAT*)(SendDlgItemMessage(hDlg, IDC_D3DC_BACK_BUFFER_FORMATS, CB_GETITEMDATA, iSelectedBackBufferFormat, 0));
	iSelectedZStencilBufferFormat = (int)(SendDlgItemMessage(hDlg, IDC_D3DC_Z_STENCIL_BUFFER_FORMATS, CB_GETCURSEL, 0, 0));
	pSelectedZStencilBufferFormat = (D3DFORMAT*)(SendDlgItemMessage(hDlg, IDC_D3DC_Z_STENCIL_BUFFER_FORMATS, CB_GETITEMDATA, iSelectedZStencilBufferFormat, 0));

	// Liste leeren
	DeleteListData(hDlg, IDC_D3DC_MULTI_SAMPLING_TYPES);
	...
	for(int iMultiSamplingType = 0; iMultiSamplingType < 16; iMultiSamplingType++)
	{
															...
															&dwTemp)))
		{
																...
																&dwTemp)))
			{
				...
				pMultiSamplingType = (D3DMULTISAMPLE_TYPE*)(MemAlloc(sizeof(D3DMULTISAMPLE_TYPE)));
				*pMultiSamplingType = aMultiSamplingType[iMultiSamplingType];

				// Listeneintrag erzeugen
				iIndex = (int)(SendDlgItemMessage(hDlg, IDC_D3DC_MULTI_SAMPLING_TYPES, CB_ADDSTRING,
					                              0, (LPARAM)(acMultiSamplingTypeString)));
				
				// Dem Eintrag den Multi-Sampling-Typ zuweisen
				SendDlgItemMessage(hDlg, IDC_D3DC_MULTI_SAMPLING_TYPES, CB_SETITEMDATA,
	...
	SendDlgItemMessage(hDlg, IDC_D3DC_MULTI_SAMPLING_TYPES, CB_SETCURSEL, 0, 0);
...
Result EnumMultiSamplingQualities(HWND hDlg)
{
	int						iSelectedAdapter;
	int						iSelectedDeviceType;
	D3DDEVTYPE*				pSelectedDeviceType;
	...
	iSelectedBackBufferFormat = (int)(SendDlgItemMessage(hDlg, IDC_D3DC_BACK_BUFFER_FORMATS, CB_GETCURSEL, 0, 0));
	pSelectedBackBufferFormat = (D3DFORMAT*)(SendDlgItemMessage(hDlg, IDC_D3DC_BACK_BUFFER_FORMATS, CB_GETITEMDATA, iSelectedBackBufferFormat, 0));
	iSelectedMultiSamplingType = (int)(SendDlgItemMessage(hDlg, IDC_D3DC_MULTI_SAMPLING_TYPES, CB_GETCURSEL, 0, 0));
	pSelectedMultiSamplingType = (D3DMULTISAMPLE_TYPE*)(SendDlgItemMessage(hDlg, IDC_D3DC_MULTI_SAMPLING_TYPES, CB_GETITEMDATA, iSelectedMultiSamplingType, 0));

	// Liste leeren
	DeleteListData(hDlg, IDC_D3DC_MULTI_SAMPLING_QUALITIES);
	...
	for(int iQuality = 0; iQuality < (int)(dwNumMultiSamplingQualities); iQuality++)
	{
		...
		SendDlgItemMessage(hDlg, IDC_D3DC_MULTI_SAMPLING_QUALITIES, CB_ADDSTRING,
			               0, (LPARAM)(acMultiSamplingQualityString));
	}

	// Höchste Qualitätsstufe auswählen
	SendDlgItemMessage(hDlg, IDC_D3DC_MULTI_SAMPLING_QUALITIES, CB_SETCURSEL,
										   ...
										   void* pContext)
{
	...
	iIndex = (int)(SendDlgItemMessage(hDlg, IDC_DSC_DRIVERS, CB_ADDSTRING, 0, (LPARAM)(pcDescription)));

	// GUID kopieren und dem Eintrag zuweisen
	pStoredGUID = (LPGUID)(MemAlloc(sizeof(GUID)));
	if(pGUID == NULL) *pStoredGUID = GUID_NULL;
	else *pStoredGUID = *pGUID;

	SendDlgItemMessage(hDlg, IDC_DSC_DRIVERS, CB_SETITEMDATA, iIndex, (LPARAM)(pStoredGUID));
...
Result EnumSoundDrivers(HWND hDlg)
{
	DeleteListData(hDlg, IDC_DSC_DRIVERS);
	DirectSoundEnumerate(DirectSoundEnumerateCallback, (void*)(hDlg));

	// Den ersten Treiber in der Liste auswählen
	SendDlgItemMessage(hDlg, IDC_DSC_DRIVERS, CB_SETCURSEL, 0, 0);
...
Result FindListElement(HWND hDlg,
						 int iDialogItem,
						 void* pData,
						 int iSizeOfData,
						 int* piOut)
...
Result SelectListElement(HWND hDlg,
						   int iDialogItem,
						   void* pData,
						   int iSizeOfData,
						   int* piOut)
{
	...
	if(FindListElement(hDlg, iDialogItem, pData, iSizeOfData, &iIndex)) return AGE_ERROR;
				   ...
				   Config* pOut)
{
	...
	ZeroMemory(pOut, sizeof(Config));
	
	// Adapter
	pOut->Direct3D.iAdapter = (int)(SendDlgItemMessage(hDlg, IDC_D3DC_ADAPTERS, CB_GETCURSEL, 0, 0));
	
	// Gerätetyp
	iIndex = (int)(SendDlgItemMessage(hDlg, IDC_D3DC_DEVICE_TYPES, CB_GETCURSEL, 0, 0));	
	pTemp = ((void*)(SendDlgItemMessage(hDlg, IDC_D3DC_DEVICE_TYPES, CB_GETITEMDATA, iIndex, 0)));
	if(pTemp != (void*)(CB_ERR)) pOut->Direct3D.DeviceType = *((D3DDEVTYPE*)(pTemp));

	// Fenster-/Vollbildmodus
	pOut->Direct3D.bWindowed = IsDlgButtonChecked(hDlg, IDC_D3DC_WINDOWED_MODE) == BST_CHECKED;

	// Videomodus
	iIndex = (int)(SendDlgItemMessage(hDlg, IDC_D3DC_VIDEO_MODES, CB_GETCURSEL, 0, 0));
	pTemp = ((void*)(SendDlgItemMessage(hDlg, IDC_D3DC_VIDEO_MODES, CB_GETITEMDATA, iIndex, 0)));
	if(pTemp != (void*)(CB_ERR)) pOut->Direct3D.VideoMode = *((D3DDISPLAYMODE*)(pTemp));

	// Bildpufferformat
	iIndex = (int)(SendDlgItemMessage(hDlg, IDC_D3DC_BACK_BUFFER_FORMATS, CB_GETCURSEL, 0, 0));
	pTemp = ((void*)(SendDlgItemMessage(hDlg, IDC_D3DC_BACK_BUFFER_FORMATS, CB_GETITEMDATA, iIndex, 0)));
	if(pTemp != (void*)(CB_ERR)) pOut->Direct3D.BackBufferFormat = *((D3DFORMAT*)(pTemp));

	// Z-Stencil-Buffer-Format
	iIndex = (int)(SendDlgItemMessage(hDlg, IDC_D3DC_Z_STENCIL_BUFFER_FORMATS, CB_GETCURSEL, 0, 0));
	pTemp = ((void*)(SendDlgItemMessage(hDlg, IDC_D3DC_Z_STENCIL_BUFFER_FORMATS, CB_GETITEMDATA, iIndex, 0)));
	if(pTemp != (void*)(CB_ERR)) pOut->Direct3D.ZStencilBufferFormat = *((D3DFORMAT*)(pTemp));

	// Multi-Sampling-Typ
	iIndex = (int)(SendDlgItemMessage(hDlg, IDC_D3DC_MULTI_SAMPLING_TYPES, CB_GETCURSEL, 0, 0));
	pTemp = ((void*)(SendDlgItemMessage(hDlg, IDC_D3DC_MULTI_SAMPLING_TYPES, CB_GETITEMDATA, iIndex, 0)));
	if(pTemp != (void*)(CB_ERR)) pOut->Direct3D.MultiSamplingType = *((D3DMULTISAMPLE_TYPE*)(pTemp));

	// Multi-Sampling-Qualität
	pOut->Direct3D.dwMultiSamplingQuality = (DWORD)(SendDlgItemMessage(hDlg, IDC_D3DC_MULTI_SAMPLING_QUALITIES, CB_GETCURSEL, 0, 0));

	// Transformation und Beleuchtung
	pOut->Direct3D.dwFlags = D3DCREATE_MULTITHREADED;
	if(IsDlgButtonChecked(hDlg, IDC_D3DC_HARDWARE_TNL) == BST_CHECKED) pOut->Direct3D.dwFlags |= D3DCREATE_HARDWARE_VERTEXPROCESSING;
	else pOut->Direct3D.dwFlags |= D3DCREATE_SOFTWARE_VERTEXPROCESSING;

	
	// DirectSound-Treiber
	iIndex = (int)(SendDlgItemMessage(hDlg, IDC_DSC_DRIVERS, CB_GETCURSEL, 0, 0));
	if(iIndex != CB_ERR) pOut->DirectSound.SoundDriverGUID = *((LPGUID)(SendDlgItemMessage(hDlg, IDC_DSC_DRIVERS, CB_GETITEMDATA, iIndex, 0)));

	// Anzahl der Bits pro Sample
	if(IsDlgButtonChecked(hDlg, IDC_DSC_8_BITS) == BST_CHECKED) pOut->DirectSound.dwNumBitsPerSample = 8;
	else pOut->DirectSound.dwNumBitsPerSample = 16;

	// Sampling-Frequenz
	if(IsDlgButtonChecked(hDlg, IDC_DSC_11025_HZ) == BST_CHECKED) pOut->DirectSound.dwSampleRate = 11025;
	else if(IsDlgButtonChecked(hDlg, IDC_DSC_22050_HZ) == BST_CHECKED) pOut->DirectSound.dwSampleRate = 22050;
...
// so weit wie möglich.
Result SetConfig(HWND hDlg,
				   Config* pConfig)
{
	// Alle Parameter in der Liste suchen und/oder auswählen
	SendDlgItemMessage(hDlg, IDC_D3DC_ADAPTERS, CB_SETCURSEL, pConfig->Direct3D.iAdapter, 0);
	EnumDeviceTypes(hDlg); SelectListElement(hDlg, IDC_D3DC_DEVICE_TYPES, &pConfig->Direct3D.DeviceType, sizeof(D3DDEVTYPE), NULL);
	CheckDlgButton(hDlg, IDC_D3DC_WINDOWED_MODE, pConfig->Direct3D.bWindowed ? BST_CHECKED : BST_UNCHECKED);
	CheckDlgButton(hDlg, IDC_D3DC_FULLSCREEN_MODE, !pConfig->Direct3D.bWindowed ? BST_CHECKED : BST_UNCHECKED);
	EnumVideoModes(hDlg); SelectListElement(hDlg, IDC_D3DC_VIDEO_MODES, &pConfig->Direct3D.VideoMode, sizeof(D3DDISPLAYMODE), NULL);
	EnumBackBufferFormats(hDlg); SelectListElement(hDlg, IDC_D3DC_BACK_BUFFER_FORMATS, &pConfig->Direct3D.BackBufferFormat, sizeof(D3DFORMAT), NULL);
	EnumZStencilBufferFormats(hDlg); SelectListElement(hDlg, IDC_D3DC_Z_STENCIL_BUFFER_FORMATS, &pConfig->Direct3D.ZStencilBufferFormat, sizeof(D3DFORMAT), NULL);
	EnumMultiSamplingTypes(hDlg); SelectListElement(hDlg, IDC_D3DC_MULTI_SAMPLING_TYPES, &pConfig->Direct3D.MultiSamplingType, sizeof(D3DMULTISAMPLE_TYPE), NULL);
	EnumMultiSamplingQualities(hDlg); if(pConfig->Direct3D.dwMultiSamplingQuality != -1) SendDlgItemMessage(hDlg, IDC_D3DC_MULTI_SAMPLING_QUALITIES, CB_SETCURSEL, pConfig->Direct3D.dwMultiSamplingQuality, 0);
	CheckDlgButton(hDlg, IDC_D3DC_HARDWARE_TNL, pConfig->Direct3D.dwFlags & D3DCREATE_HARDWARE_VERTEXPROCESSING ? BST_CHECKED : BST_UNCHECKED);
	CheckDlgButton(hDlg, IDC_D3DC_SOFTWARE_TNL, pConfig->Direct3D.dwFlags & D3DCREATE_SOFTWARE_VERTEXPROCESSING ? BST_CHECKED : BST_UNCHECKED);

	SelectListElement(hDlg, IDC_DSC_DRIVERS, &pConfig->DirectSound.SoundDriverGUID, sizeof(GUID), NULL);
	CheckDlgButton(hDlg, IDC_DSC_8_BITS, pConfig->DirectSound.dwNumBitsPerSample == 8 ? BST_CHECKED : BST_UNCHECKED);
	CheckDlgButton(hDlg, IDC_DSC_16_BITS, pConfig->DirectSound.dwNumBitsPerSample == 16 ? BST_CHECKED : BST_UNCHECKED);
	CheckDlgButton(hDlg, IDC_DSC_11025_HZ, pConfig->DirectSound.dwSampleRate == 11025 ? BST_CHECKED : BST_UNCHECKED);
	CheckDlgButton(hDlg, IDC_DSC_22050_HZ, pConfig->DirectSound.dwSampleRate == 22050 ? BST_CHECKED : BST_UNCHECKED);
	CheckDlgButton(hDlg, IDC_DSC_44100_HZ, pConfig->DirectSound.dwSampleRate == 44100 ? BST_CHECKED : BST_UNCHECKED);
								   ...
								   LPARAM LParam)
{
	...
	case WM_INITDIALOG:
		...
		CheckDlgButton(hDlg, IDC_D3DC_FULLSCREEN_MODE, BST_CHECKED);
		CheckDlgButton(hDlg, IDC_D3DC_HARDWARE_TNL, BST_CHECKED);
		CheckDlgButton(hDlg, IDC_DSC_16_BITS, BST_CHECKED);
		CheckDlgButton(hDlg, IDC_DSC_44100_HZ, BST_CHECKED);
	...
	case WM_COMMAND:		
		...
		case IDC_D3DC_ADAPTERS:
			...
			else
			{
				...
				SetDlgItemText(hDlg, IDC_C_DESCRIPTION, D3DC_ADAPTER_DESCRIPTION);
			}
			break;

		case IDC_D3DC_DEVICE_TYPES:
			if(HIWORD(WParam) == CBN_SELCHANGE)
			{
			...
			{
				// Informationstext anzeigen
				SetDlgItemText(hDlg, IDC_C_DESCRIPTION, D3DC_DEVICE_TYPE_DESCRIPTION);
			}
			break;

		case IDC_D3DC_WINDOWED_MODE:
		case IDC_D3DC_FULLSCREEN_MODE:
			...
			// Informationstext anzeigen
			SetDlgItemText(hDlg, IDC_C_DESCRIPTION, D3DC_WINDOWED_FULLSCREEN_DESCRIPTION);
			break;

		case IDC_D3DC_VIDEO_MODES:
			if(HIWORD(WParam) == CBN_SELCHANGE)
			{
			...
			{
				// Informationstext anzeigen
				SetDlgItemText(hDlg, IDC_C_DESCRIPTION, D3DC_VIDEO_MODE_DESCRIPTION);
			}
			break;

		case IDC_D3DC_BACK_BUFFER_FORMATS:
			if(HIWORD(WParam) == CBN_SELCHANGE)
			{
			...
			{
				// Informationstext anzeigen
				SetDlgItemText(hDlg, IDC_C_DESCRIPTION, D3DC_BACK_BUFFER_FORMAT_DESCRIPTION);
			}
			break;

		case IDC_D3DC_Z_STENCIL_BUFFER_FORMATS:
			if(HIWORD(WParam) == CBN_SELCHANGE)
			{
			...
			{
				// Informationstext anzeigen
				SetDlgItemText(hDlg, IDC_C_DESCRIPTION, D3DC_Z_STENCIL_BUFFER_FORMAT_DESCRIPTION);
			}
			break;

		case IDC_D3DC_MULTI_SAMPLING_TYPES:
			if(HIWORD(WParam) == CBN_SELCHANGE)
			{
			...
			{
				// Informationstext anzeigen
				SetDlgItemText(hDlg, IDC_C_DESCRIPTION, D3DC_MULTI_SAMPLING_TYPE_DESCRIPTION);
			}
			break;

		case IDC_D3DC_MULTI_SAMPLING_QUALITIES:
			if(HIWORD(WParam) != CBN_SELCHANGE)
			{
				// Informationstext anzeigen
				SetDlgItemText(hDlg, IDC_C_DESCRIPTION, D3DC_MULTI_SAMPLING_QUALITY_DESCRIPTION);
			}
			break;

		case IDC_D3DC_HARDWARE_TNL:
		case IDC_D3DC_SOFTWARE_TNL:
			// Informationstext anzeigen
			SetDlgItemText(hDlg, IDC_C_DESCRIPTION, D3DC_TNL_DESCRIPTION);
			break;

		case IDC_DSC_DRIVERS:
			// Informationstext anzeigen
			SetDlgItemText(hDlg, IDC_C_DESCRIPTION, DSC_DRIVERS_DESCRIPTION);
			break;

		case IDC_DSC_8_BITS:
		case IDC_DSC_16_BITS:
			// Informationstext anzeigen
			SetDlgItemText(hDlg, IDC_C_DESCRIPTION, DSC_BITS_PER_SAMPLE_DESCRIPTION);
			break;

		case IDC_DSC_11025_HZ:
		case IDC_DSC_22050_HZ:
		case IDC_DSC_44100_HZ:
			// Informationstext anzeigen
			SetDlgItemText(hDlg, IDC_C_DESCRIPTION, DSC_SAMPLING_FREQUENCY_DESCRIPTION);
			break;

		case IDC_C_SAVE:
			{
				strcpy(acFilename, "*.c");
				OPENFILENAME Filename = {sizeof(OPENFILENAME), hDlg, NULL, "C-Dateien\0.c", NULL,
										 0, 1, acFilename, 255, NULL, 0, NULL, "C-Datei speichern",
				...
				if(GetSaveFileName(&Filename))
				{
					...
					fwrite(&g_TempConfig, sizeof(Config), 1, pFile);
					fclose(pFile);
				}
			}
			break;
		...
		case IDC_C_LOAD:
			{
				strcpy(acFilename, "*.c");
				OPENFILENAME Filename = {sizeof(OPENFILENAME), hDlg, NULL, "C-Dateien\0.c\0Alle Dateien\0*.*", NULL,
										 0, 1, acFilename, 255, NULL, 0, NULL, "C-Datei laden",
				...
				if(GetOpenFileName(&Filename))
				{
					...
					GetConfig(hDlg, &g_TempConfig);
					fread(&g_TempConfig, sizeof(Config), 1, pFile);
					fclose(pFile);
					SetConfig(hDlg, &g_TempConfig);
				}
			}
...
Result DoConfigDialog(Config* pOut)
{
	int iResult;
	...
	AGE_INFO("Der Konfigurationsdialog wird aufgerufen...");
	...
	if(g_pD3DTemp == NULL)
	{
		...
		AGE_ERROR("Die IDirect3D9-Schnittstelle konnte nicht generiert werden! DirectX 9 oder höher wird benötigt!", AGE_ERROR);
	}

	// Dialogfenster aufrufen
	iResult = (int)(DialogBox(g_DLLHandle,
							  MAKEINTRESOURCE(IDD_CONFIG),
						      NULL,
						      D3DEnumDialogProc));

	// Die Schnittstelle wieder freigeben
	AGE_SAFE_RELEASE(g_pD3DTemp);

	// Rückgabewert des Dialogfensters auswerten
	if(iResult == 0) return AGE_OK;					// Alles OK!
	else if(iResult == 1) return AGE_ERROR;			// Fehler!
	else /*if(iResult == 2)*/ return AGE_CANCELED;	// Dialog abgebrochen
}