...
public class BootStrapedApplicationStarter implements BootStrapedComponent {
     ...
     * - starten der Koponente abhängig von ihrem Typ
     * - Regisrierung der Komponente
     */
    public void init(final Properties p) {
        ...
        try {
            compClass = RMIClassLoader.loadClass(p.getProperty("codebase"), p.getProperty("compClassName"));
            System.out.println("BootStrapedApplicationStarter.init() After loading Component Class");
        } catch (MalformedURLException e) {
            e.printStackTrace();
            System.out.println("BootStrapedApplicationStarter.init()" + e.getMessage());
        ...
        if (p.getProperty("typ").equals("server")) {
            ...
            runClient();
        } else if (p.getProperty("typ").equals("admin")) {
            System.out.println("model.BootStrapedClientStarter --- Admin");
            runClient();
        }
    ...
    private void runClient() {
        ...
        try {
            System.out.println("\tlookup for " + component.getRmiName() + " =  compName >" + name);
            server = (ServerHandle)Naming.lookup(component.getRmiName() + name);
            System.out.println("\tlookup(" + component.getRmiName() + name + " = " + server);
            ...
            System.out.println("\tEXPORTIERT = " + client);
			component.setApp(client);
            System.out.println("\tin Service registriert = " + component);
            System.out.println("\tbei Server registriert = " + server);
        } catch (Exception e) {
            e.printStackTrace();
            System.out.println("Fehler in BootStrapedApplicationStarter.runClient() > UUHUHUHUHU >> " + e.fillInStackTrace() +
                "\n" + e.getMessage());
        }
        System.out.println("<= BootStrapedApplicationStarter.runClient()");
    }
    ...
    private void runServer() {
        ...
        try {
            ...
            System.out.println("\tcreateRegistry(Integer.parseInt(rmiPort) =" + component.getPort() + " compName >" + name);
            ...
            System.out.println("\nNaming.rebind out > " + component.getRmiName() + "getProp(compName) " + component.getCompName() +
            ...
            e.printStackTrace();
            System.out.println("Fehler in BootStrapedServerStarter.runServer() : " + e.getMessage());
        }
        server = (ServerHandle)serverObject; // für register Methode erforderlich
        System.out.println("<= BootStrapedApplicationStarter.runServer()");
    }

    /**
     * Ruft abhängig von Authentifizierungsart die entsprechende <code>register</code> Methode auf.
     */
    protected void register(final RemoteObject remoteObject) {
        System.out.println("=> BootStrapedApplicationStarter.register(RemoteObjectProperties " + remoteObject +
            " , ServerHandle " + server + ")");
        // TODO Prüfen der Registermethode
        try {
            if (remoteObject.getAuthTyp().equals("nothing")) {
                System.out.println("\tKeine Registrierung erforderlich > authTyp = nothing");
            } else if (remoteObject.getAuthTyp().equals("anonym")) {
                System.out.println("\tAnonyme Registrierung > authTyp = anonym > an : " + server + "  > mit : " + remoteObject);
                server.register(remoteObject);
            } else if (remoteObject.getAuthTyp().equals("password")) {
                System.out.println("\tRegistrierung mit Authentifikation > authTyp = password");
                server.register(remoteObject, remoteObject.getProps().getProperty("username"), remoteObject.getProps().getProperty("passwd"));
            }
        } catch (RemoteException e) {
            System.out.println("Fehler inBootStrapedApplicationStarter.register(RemoteObjectProperties " + remoteObject +
                " , ServerHandle " + server + " : " + e.getMessage());
        }
        System.out.println("<= BootStrapedApplicationStarter.register(RemoteObjectProperties " + remoteObject +
            " , ServerHandle " + server + ")");
    }
