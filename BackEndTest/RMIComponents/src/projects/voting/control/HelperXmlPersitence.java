...
public class HelperXmlPersitence implements PersistenceHelper {
	...
	public HelperXmlPersitence(String _dateiname) {
		System.out.println("=>HelperXmlPersitence.HelperXmlPersitence(dateiname: "+_dateiname+" )");
		dateiname = _dateiname;
		

		// Nestedconfig parser einstellen
		System
				.setProperty("jconfig.parser", NestedConfigParser.class
						.getName());
		// Errorhandler gibt fehlermeldungen auf der console aus
		System.setProperty("jconfig.errorhandler",
				"org.jconfig.error.SimpleErrorHandler");
	 ...
	 */
	public void save() {
		System.out.println("=>HelperXmlPersitence.save()");
		//  Nestedconfig parser einstellen
		System
				.setProperty("jconfig.parser", NestedConfigParser.class
						.getName());
	...
	/**
	 * Holt die Vote Obj. aus dem Hashtable, liest den count und speichert ihn
	 * in der Configuration (jconfig)
	 *  
	 */
	public void refreshConfiguration() {
		...
		for (int i = 0; i < categorynames.length; i++) {
			...
			if (categorynames[i] != "general") {
				...
				configuration.setIntProperty("count", tmpvt.getCount(),
						categorynames[i]);
			}
		}
	}
	...
	public VoteTable getVoteTable() {
		System.out.println("=>HelperXmlPersitence.getVoteTable()");
		if (votes == null) {
			votes = new VoteTable();
			...
			for (int i = 0; i < categorynames.length; i++) {
				...
				System.out.println(" Anzahl: "
						+ configuration.getIntProperty("count", 0,
								categorynames[i]));

				//da es immer die general Kategorie gibt darf die NICHT
				// mitspielen
				if (categorynames[i] != "general") {
					Vote vote = new Vote();
					vote.setDescription(configuration.getProperty(
							"description", "", categorynames[i]));
					vote.setCount(configuration.getIntProperty("count", -1,
							categorynames[i]));
	...
	public List listVotes() {
		for (int i = 0; i < categorynames.length; i++) {
			...
			System.out.println(" Anzahl: "
					+ configuration
							.getIntProperty("count", 0, categorynames[i]));
		}
		return null;
	}
